# Single host inference benchmark of Llama3.1-70B with vLLM on G4

This recipe shows how to serve and benchmark Llama3.1-70B model using [vLLM](https://github.com/vllm-project/vllm) on a single GCP VM with G4 GPUs. vLLM is a high-throughput and memory-efficient inference and serving engine for LLMs. For more information on G4 machine types, see the [GCP documentation](https://cloud.google.com/compute/docs/accelerator-optimized-machines#g4-machine-types).
...
### 1. Scenario A: Serving on Single-Chip (1-GPU)

To run the vLLM server, you can use the following command:

```bash
# Set TP=1 for a single GPU
sudo docker run \
    --gpus all \
...
-   `--model nvidia/Llama-3.1-70B-Instruct-FP8`: The model to be served from Hugging Face.
-   `--kv-cache-dtype fp8`: Sets the data type for the key-value cache to FP8 to save GPU memory.
-   `--gpu-memory-utilization 0.90`: The fraction of GPU memory to be used by vLLM.
-   `--tensor-parallel-size 1`: It specifies the number of gpu's to use.

### 2. Scenario B: Serving on Multi-Chip (8-GPU): 

The SYS value for NCCL_P2P_LEVEL instructs NCCL to use P2P communication for all GPUs, including those separated by the system's CPU/NUMA interconnects, ensuring the fastest possible data transfer across the entire multi-GPU system.
...
Review Google Cloud's announcement on the enhanced Peer-to-Peer communication fabric designed specifically for G4 VMs to accelerate multi-GPU workloads.

Reference: [GCP P2P documentation](https://cloud.google.com/blog/products/compute/g4-vms-p2p-fabric-boosts-multi-gpu-workloads/)

we will use the official vllm/vllm-openai:latest Docker image to run the server. 

```bash
# Set TP=8 to shard the 70B model
sudo docker run \
    --gpus all \
    -v ~/.cache/huggingface:/root/.cache/huggingface \
    --env "HUGGING_FACE_HUB_TOKEN=$HF_TOKEN" \
...
## Run Benchmarks for nvidia/Llama-3.1-70B-Instruct-FP8
...
sudo docker run \
    ...
    --model nvidia/nvidia/Llama-3.1-70B-Instruct-FP8 \
    --dataset-name random \
...
Here's a breakdown of the arguments:
- `--model nvidia/nvidia/Llama-3.1-70B-Instruct-FP8`: The model to benchmark.
